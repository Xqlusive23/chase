  ...
  return (
    <div className="mx-auto max-w-xl space-y-4">
      <div className="soft-card flex items-center gap-4 p-5">
        <button type="button" onClick={() => photoRef.current?.click()} aria-label="Change profile photo">
        ...
        <div className="min-w-0">
          ...
          <button type="button" onClick={() => photoRef.current?.click()} className="mt-2 text-sm font-semibold text-[var(--blue)]">
            Change photo
          </button>
        </div>
        <input ref={photoRef} type="file" accept="image/*" capture="user" className="hidden" onChange={(event) => handlePhoto(event.target.files?.[0])} />
      </div>
      {LINKS.map((item) => (
